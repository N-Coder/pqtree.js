...
double computeCircularWeight(pc_tree::PCTree &tree,
                             pc_tree::PCTreeNodeArray<double> &weights,
                             double inner_weight, double leaf_weight,
                             std::function<double(double, double)> fold);

inline double
computeCircularWeightByHeight(pc_tree::PCTree &tree,
                              pc_tree::PCTreeNodeArray<double> &weights) {
  return computeCircularWeight(tree, weights, 0, 1, [](double c, double n) {
    return std::max(c + 1, n);
  });
}

inline double
computeCircularWeightBySubtreeSize(pc_tree::PCTree &tree,
                                   pc_tree::PCTreeNodeArray<double> &weights,
                                   double inner_weight = 1) {
 ...
 */
void computePositionsCircular(
    pc_tree::PCTree &tree, Layout &positions, double radius,
    pc_tree::PCTreeNodeArray<double> *weights = nullptr, double off_x = 0,
    double off_y = 0);
